...
| Id  | Change                                                 | Description                                                                                                                                                                                                             |                                            
|-----|--------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|  1  |**Removing or renaming a path, field, header, operationId or enum value**    |Client code that refers to API elements that have been renamed or removed can break on calling the API, therefore renaming or removing API elements **MUST** result in a major client version change.|
...
|  12  | **Adding or removing a value in an enum**                          |Enum is used for unchanging lists hence any change to the contents of an enum are considered a breaking change and therefore **MUST** result in a major version change.|of payload A (API version 1.0.0). If a GET operation is made against the server then fields A and B are returned and the client must be tolerant of the existence of field B by ignoring it. 
